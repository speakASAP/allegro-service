...
- **Symptom**: On `https://allegro.statex.cz/login`, the browser shows:
  - `🔴 Service Connection Error` with `{ service: 'API Gateway', port: ${API_GATEWAY_PORT:-3411}, url: 'http://localhost:${API_GATEWAY_PORT:-3411}/api/auth/login', error: 'Network Error', code: 'ERR_NETWORK' }` (port configured in `allegro/.env`)
  - Network tab shows `POST http://localhost:${API_GATEWAY_PORT:-3411}/api/auth/login net::ERR_CONNECTION_REFUSED`.
- **Server state**:
  - `allegro-api-gateway-green` is running and healthy, mapped as `127.0.0.1:${API_GATEWAY_PORT:-3411}->${API_GATEWAY_PORT:-3411}/tcp`.
  - `curl http://localhost:${API_GATEWAY_PORT:-3411}/health` on the server returns a healthy response.
- **Codebase references**:
  - API Gateway docs: `README.md` (API Gateway on port ${API_GATEWAY_PORT:-3411}, all traffic via `http://localhost:${API_GATEWAY_PORT:-3411}/api`, port configured in `allegro/.env`).
...
- The frontend bundle currently served for `https://allegro.statex.cz` is using the **development fallback** API base URL:
  - `http://localhost:${API_GATEWAY_PORT:-3411}/api` (port configured in `allegro/.env`)
...
1. Ensure `.env` (local and production) contains correct, non-localhost configuration:
   - `API_GATEWAY_PORT=3411` (configured in `allegro/.env`)
   - `ALLEGRO_FRONTEND_SERVICE_PORT=3410` (configured in `allegro/.env`)
...
4. Verify nginx reverse proxy for `allegro.statex.cz`:
   - `location /` routes to `allegro-frontend-service` (port ${ALLEGRO_FRONTEND_SERVICE_PORT:-3410}, configured in `allegro/.env`).
...
1. ✅ Confirm `services/allegro-frontend-service/src/services/api.ts` uses:
   ...
   - `http://localhost:${API_GATEWAY_PORT:-3411}/api` (port configured in `allegro/.env`) only as a development fallback.
...
4. ✅ Fixed API Gateway connectivity to auth service - changed `AUTH_SERVICE_URL` from `https://auth.statex.cz` to `http://auth-microservice:${PORT:-3370}` (Docker network, port configured in `auth-microservice/.env`).
...
4. ✅ Fixed settings service connectivity - updated `SETTINGS_SERVICE_URL=http://allegro-settings-service:${ALLEGRO_SETTINGS_SERVICE_PORT:-3408}` (Docker network, port configured in `allegro/.env`).
